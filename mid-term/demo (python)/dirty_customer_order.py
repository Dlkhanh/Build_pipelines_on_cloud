...
# List of special characters to add to the names
special_chars = ['@', '#', '$', '%', '^', '&', '*', '(', ')', '-', '_', '+', '=', '~', '`', '|', '\\', '{', '}', '[', ']', ':', ';', '"', "'", '<', '>', ',', '.', '?', '/']

# Function to generate a name with special characters and spaces
def generate_special_name():
    name = fake.name()
    # Add random special characters and extra spaces
    name_parts = list(name)
    for _ in range(random.randint(1, 3)):
        position = random.randint(0, len(name_parts))
        name_parts.insert(position, random.choice(special_chars))
        name_parts.insert(position, ' ' * random.randint(1, 5))
    special_name = ''.join(name_parts)
    return special_name[:40]  # Ensure the name length does not exceed 40 characters
...
for _ in range(30):
    ...
    receiver_name = generate_special_name()  # Generate special name
